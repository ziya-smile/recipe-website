  ...
  <li>
    <RouterLink class="recipe-card" :to="`/recipes/${recipe.id}`">
      ...
      <div class="recipe-card-body">
        <div v-if="recipe.category || recipe.cook_time || recipe.difficulty" class="card-badges">
          <span v-if="recipe.category" class="card-badge">📂 {{ recipe.category }}</span>
          <span v-if="recipe.cook_time" class="card-badge">⏱️ {{ recipe.cook_time }}m</span>
          <span v-if="recipe.difficulty" class="card-badge">{{ recipe.difficulty === 'easy' ? '🟢' : recipe.difficulty === 'medium' ? '🟡' : '🔴' }} {{ recipe.difficulty }}</span>
        </div>
        <h2>{{ recipe.title }}</h2>
